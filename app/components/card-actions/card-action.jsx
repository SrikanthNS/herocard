 ...
 */
 export default class CardActionComponent extends Component {
    constructor(props) {
        super(props);
    ...
    userInputSectionClasses(userInput) {
        ...
        if (userInput.length < 2) {
            return classNames += ' hccf-card-actions__add-input-single';
        }
        
        return classNames += ' hccf-card-actions__add-input-multiple';
    }
    ...
    render() {
        ...
        return (
            <div id={action.id} className={this.actionItemClasses(numActions, action)} >
                <form id={`${cardName}${index}`}  
                    method={action.type} className="hccf-card-action-form"
                    ...
                    {_.map(action.request, (propValue, propName) => {
                        return <HiddenField key={propName} name={propName} value={propValue} />
                    })}
